...
function fn_test_22 {
	...
	"$f_STAT_WATCH" --config "" --backup-file "$d_STATWATCH_TESTS_WORKING/testing/123?456.php" --backupd "$d_STATWATCH_TESTS_WORKING"/testing2/backup
	...
	sleep 1.1
	"$f_STAT_WATCH" --config "" --backup-file "$d_STATWATCH_TESTS_WORKING/testing/123?456.php" --backupd "$d_STATWATCH_TESTS_WORKING"/testing2/backup --hold
	if [[ $( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING"/testing | egrep -c "123\?456\.php_[0-9]+$" ) -ne 2 ]]; then
		fn_fail "22.5.1"
	fi
	...
	"$f_STAT_WATCH" --config "" --backup-file "$d_STATWATCH_TESTS_WORKING/testing/123?456.php" --backupd "$d_STATWATCH_TESTS_WORKING"/testing2/backup --comment 'very nice!'
	if [[ $( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING"/testing | egrep -c "123\?456\.php_[0-9]+$" ) -ne 2 ]]; then
		fn_fail "22.6.1"
	fi
	...
	if [[ $( "$f_STAT_WATCH" --config "" --list "$d_STATWATCH_TESTS_WORKING/testing/123?456.php" | egrep -c "$( fn_sanitize "$f_BACKUP3" ).* -- HELD" ) -ne 1 ]]; then
		fn_fail "22.7"
	fi
	fn_pass "22.7"

	### "--list" should also show the comment
	if [[ $( "$f_STAT_WATCH" --config "" --list "$d_STATWATCH_TESTS_WORKING/testing/123?456.php" | egrep -A1 "$( fn_sanitize "$f_BACKUP3" ).* -- HELD" | egrep -c "very nice" ) -ne 1 ]]; then
		fn_fail "22.8"
	fi
	fn_pass "22.8"
}
