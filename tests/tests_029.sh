...
function fn_test_29 {
	...
	"$f_STAT_WATCH" --config "$f_CONF" --backup-file "$d_STATWATCH_TESTS_WORKING/testing/123.php" --backupd "$d_STATWATCH_TESTS_WORKING"/testing2/backup
	if [[ $( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing" | egrep -c "123.*md5$" ) -ne 0 ]]; then
		fn_fail "29.1"
	fi
	fn_pass "29.1"
	...
	"$f_STAT_WATCH" --config "$f_CONF" --backup-file "$d_STATWATCH_TESTS_WORKING/testing/123.php" --backupd "$d_STATWATCH_TESTS_WORKING"/testing2/backup
	if [[ $( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing" | egrep -c "123.*md5$" ) -ne 2 ]]; then
		fn_fail "29.2.1"
	fi
	if [[ $( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing" | egrep -c "123\.php_[0-9]+$" ) -ne 3 ]]; then
		fn_fail "29.2.2"
	fi
	fn_pass "29.2"

	### If those two backups turn out to have the same content, will a pointer file be correctly made
	v_FILE="$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing/""$( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing" | egrep "_pointer" )"
	v_FILE="$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing/123.php_""$( cat "$v_FILE" )"
	...
	v_FILE="$d_STATWATCH_TESTS_WORKING/testing/123.php_""$( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing" | egrep "_pointer$" | sort | head -n1 | rev | cut -d "_" -f2 | rev )"
	"$f_STAT_WATCH" --config "$f_CONF" --restore "$v_FILE"
	if [[ "$( stat -c %a "$d_STATWATCH_TESTS_WORKING/testing/123.php" )" != "644" ]]; then
		fn_fail "29.4.1"
	fi
	v_FILE="$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing/""$( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing" | egrep "_[0-9]+$" | sort | tail -n1 )"
	if [[ "$( cat "$v_FILE" )" != "SASQUATCH" ]]; then
		fn_fail "29.4.2"
	fi
	fn_pass "29.4"
	...
	v_FILE="$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing/123.php_""$( \ls -1 "$d_STATWATCH_TESTS_WORKING"/testing2/backup"$d_STATWATCH_TESTS_WORKING/testing" | egrep "_pointer$" | sort | head -n2 | tail -n1 | rev | cut -d "_" -f2 | rev )"
	"$f_STAT_WATCH" --config "$f_CONF" --restore "$v_FILE"
