...
function fn_test_29 {
	...
	chmod 0644 "$d_PROGRAM_TESTS_WORKING/testing/123.php"
	...
	chmod 0600 "$d_PROGRAM_TESTS_WORKING/testing/123.php"
	"$f_STAT_WATCH" --config "$f_CONF" --backup-file "$d_PROGRAM_TESTS_WORKING/testing/123.php" --backupd "$d_PROGRAM_TESTS_WORKING"/testing2/backup
	...
	sleep 1.1
	chmod 0611 "$d_PROGRAM_TESTS_WORKING/testing/123.php"
	"$f_STAT_WATCH" --config "$f_CONF" --backup-file "$d_PROGRAM_TESTS_WORKING/testing/123.php" --backupd "$d_PROGRAM_TESTS_WORKING"/testing2/backup

	### If we attempt to restore from the first of the two pointer files, is the correct content restored
	v_FILE="$d_PROGRAM_TESTS_WORKING/testing/123.php_""$( \ls -1 "$d_BACKUP" | egrep "_pointer$" | sort | head -n1 | rev | cut -d "_" -f2 | rev )"
	"$f_STAT_WATCH" --config "$f_CONF" --restore "$v_FILE"
	if [[ "$( stat -c %a "$d_PROGRAM_TESTS_WORKING/testing/123.php" )" != "644" ]]; then
		fn_fail "29.4.1"
	fi
	...
	sleep 1.1
	chmod 0622 "$d_PROGRAM_TESTS_WORKING/testing/123.php"
	"$f_STAT_WATCH" --config "$f_CONF" --backup-file "$d_PROGRAM_TESTS_WORKING/testing/123.php" --backupd "$d_PROGRAM_TESTS_WORKING"/testing2/backup
	...
	"$f_STAT_WATCH" --config "$f_CONF" --restore "$v_FILE"
	if [[ "$( stat -c %a "$d_PROGRAM_TESTS_WORKING/testing/123.php" )" != "600" ]]; then
		fn_fail "29.5.1"
	fi
	v_FILE="$d_BACKUP/""$( \ls -1 "$d_BACKUP" | egrep "_[0-9]+$" | sort | tail -n1 )"
	if [[ "$( cat "$v_FILE" )" != "SASQUATCH" ]]; then
	...
	rm -f "$d_PROGRAM_TESTS_WORKING/testing/subdir/123 456.php"
	ln -sf "$d_PROGRAM_TESTS_WORKING/testing" "$d_PROGRAM_TESTS_WORKING/testing/subdir/123 456.php"
	"$f_STAT_WATCH" --config "$f_CONF" --backup-file "$d_PROGRAM_TESTS_WORKING/testing/subdir/123 456.php" --backupd "$d_PROGRAM_TESTS_WORKING"/testing2/backup
