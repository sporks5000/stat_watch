...
function fn_test_7 {
	...
	fn_md5_modules
	"$f_STAT_WATCH" --config "" --record "$d_STATWATCH_TESTS_WORKING"/testing --output "$d_STATWATCH_TESTS_WORKING"/testing2/report1.txt --md5
	...
	"$f_STAT_WATCH" --config "" "$d_STATWATCH_TESTS_WORKING"/testing -i <( echo "MD5 $d_STATWATCH_TESTS_WORKING/testing/123'456.php" ) --output "$d_STATWATCH_TESTS_WORKING"/testing2/report1.txt
	...
	"$f_STAT_WATCH" --config "" "$d_STATWATCH_TESTS_WORKING"/testing -i <( echo "MD5R \.txt$" ) --output "$d_STATWATCH_TESTS_WORKING"/testing2/report1.txt
	...
	if [[ $EUID -ne 0 ]]; then
		...
		fn_make_user
		sudo -u "$v_USER" "$d_USER_STATWATCH"/stat_watch_wrap.sh --config "" --record "$d_USER_STATWATCH"/tests/working/testing --output "$d_USER_STATWATCH"/tests/working/testing2/report1.txt --md5
