...
function fn_test_8 {
	...
	sleep 1.1
	"$f_STAT_WATCH" --config "$f_CONF" "$d_STATWATCH_TESTS_WORKING"/testing -i <( echo "Max-depth 0" ) --output "$d_STATWATCH_TESTS_WORKING"/testing2/report2.txt
	if [[ $( "$f_STAT_WATCH" --config "$f_CONF" --diff "$d_STATWATCH_TESTS_WORKING"/testing2/report1.txt "$d_STATWATCH_TESTS_WORKING"/testing2/report2.txt | egrep -c "DIRECTORIES TOO DEEP TO PROCESS" ) -ne 0 ]]; then
		fn_fail "8.2"
	fi
	fn_pass "8.2"
	...
	"$f_STAT_WATCH" --config "$f_CONF" "$d_STATWATCH_TESTS_WORKING"/testing -i <( echo "Max-depth 0" ) --output "$d_STATWATCH_TESTS_WORKING"/testing2/report2.txt
	if [[ $( "$f_STAT_WATCH" --config "$f_CONF" --diff "$d_STATWATCH_TESTS_WORKING"/testing2/report2.txt "$d_STATWATCH_TESTS_WORKING"/testing2/report1.txt | egrep -A2 "DIRECTORIES TOO DEEP TO PROCESS" | egrep -c "testing/subdir" ) -ne 2 ]]; then
		fn_fail "8.3"
	fi
	fn_pass "8.3"
}
