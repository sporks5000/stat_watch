...
function fn_test_5 {
	...
	if [[ $( "$f_STAT_WATCH" --config "$f_CONF" --record "$d_PROGRAM_TESTS_WORKING"/testing --as-dir /home | egrep -c "Processing: '/home'" ) -ne 1 ]]; then
		fn_fail "5.1"
	fi
	fn_pass "5.1"

	### Make sure that it's accurately labeling the root directory for the search as the new directory
	if [[ $( "$f_STAT_WATCH" --config "$f_CONF" --record "$d_PROGRAM_TESTS_WORKING"/testing --as-dir /home | egrep -c "^'/home' -- " ) -ne 1 ]]; then
		fn_fail "5.2"
	fi
	fn_pass "5.2"

	### Make sure that all files are being accurately labeled as existing in the new directory
	if [[ $( "$f_STAT_WATCH" --config "$f_CONF" --record "$d_PROGRAM_TESTS_WORKING"/testing --as-dir /home | egrep -c "^'/home" ) -ne 18 ]]; then
		fn_fail "5.3"
	fi
	fn_pass "5.3"
}
